...
LOG_FILE = open("/netscratch/nham/logs/training_debug_log_ppo_4.txt", "w", encoding="utf-8")
def log(*args):
...
BEST_MODEL_PATH = os.path.join(CKPT_DIR, "best_model_4")
...
def plot_det_critic_vs_target_return(epoch: int, det_rewards, det_values, det_dones,
                                     gamma: float, gae_lambda: float, outdir="pics-ppo-3"):
    """
    Plot:
...
cfg = ColdSprayConfig(grid_n=128, rho_m=6500.0, eta=0.4, target_height=0.2,
                      overfill_ratio=1.2, dx_max=0.05, dy_max=0.05, dt=2.0, 
                      f_init=1.0, max_steps=200000, sigma=0.02, beta=1.0, seed=None)
...
if os.path.exists("/netscratch/nham/checkpoints/best_model_4.zip"):
...
def train_sb3(env_cfg, model, N_epochs=10, log_path="training_log_ppo_4.txt"):
    ...
    for epoch in range(N_epochs):
        ...
        if mean_h_inside > best_mean_h:
            ...
            plot_det_critic_vs_target_return(
                ...
                outdir="/netscratch/nham/logs/pics-ppo-4")
...
if __name__ == "__main__":
    ...
    train_sb3(cfg, model, N_epochs=500, log_path="training_log_ppo_4.txt")