...
LOG_FILE = open("/netscratch/nham/logs/training_debug_log_ppo_3.txt", "w", encoding="utf-8")
def log(*args):
...
BEST_MODEL_PATH = os.path.join(CKPT_DIR, "best_model_3")
...
class DepositionModel:
    ...
    def apply(self, height, x0, y0, x1, y1, f, dt, use_target_mask: bool = True):
...
class ColdSprayEnv(gym.Env):
    ...
    def step(self, action):
        ...
        delta_h = self.deposition.apply(self.height, x0, y0, x1, y1, f, self.cfg.dt, use_target_mask=True)
        self.height = (self.height + delta_h).astype(np.float32, copy=False)
        self.x, self.y, self.f = x1, y1, f

        # Reward shaping: min height PROGRESS (monotonic improvement)
        if np.any(inside_box):
            h_inside_after = self.height[inside_box]
            min_after = float(np.percentile(h_inside_after, 1.0))
        else:
            min_after = 0.0

        h_target = float(self.cfg.target_height)
        dmin = (min_after - min_before)/h_target

        w_min = 1.0
        if mu_before < h_target:
            r_progress = w_min * max(0.0, dmin)
            reward += r_progress

        # ===== Reward shaping: mean height progress (monotonic improvement) =====
        mu_after = float(np.mean(self.height[inside_box])) if np.any(inside_box) else 0.0
        h_target = float(self.cfg.target_height)

        w_mu = 1.0
        if mu_before < h_target:
            r_mean = w_mu * max(0.0, (mu_after - mu_before)/h_target)
            reward += r_mean
        ...
        viol_outside = np.any(h[outside_box] > 0.1)
...
cfg = ColdSprayConfig(grid_n=128, rho_m=6500.0, eta=0.4, target_height=0.2,
                      overfill_ratio=1.01, dx_max=0.5, dy_max=0.5, dt=2.0, f_init=1.0, 
                      f_min=0.05, max_steps=200000, sigma=0.02, beta=1.0, seed=None)
...
if os.path.exists("/netscratch/nham/checkpoints/best_model_3.zip"):
...
def train_sb3(env_cfg, model, N_epochs=10, log_path="training_log_ppo_3.txt"):
    ...
    for epoch in range(N_epochs):
        ...
        if rmse < best_rmse:
            ...
            save_exploration_vs_deterministic(
                ...
                outdir="/netscratch/nham/logs/pics-ppo-3")
...
if __name__ == "__main__":
    ...
    train_sb3(cfg, model, N_epochs=500, log_path="training_log_ppo_3.txt")