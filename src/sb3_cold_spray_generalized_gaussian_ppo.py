...
LOG_FILE = open("/netscratch/nham/logs/training_debug_log_ppo.txt", "w", encoding="utf-8")
def log(*args):
...
class DepositionModel:
    def __init__(self, grid_n: int, sigma: float = 0.02, beta: float = 2.0, eta: float = 1.0, rho_m: float = 1.0, samples_per_pixel: float = 1.0,
                 A_amp: float = 1.0, overfill_ratio: float = 1.1, target_height: float = 0.2) -> None:
        self.n = int(grid_n)
...
class ColdSprayEnv(gym.Env):
    ...
    def __init__(self, config: Optional[ColdSprayConfig] = None) -> None:
        ...
        self.x, self.y = 0.5, 0.5; self.f = 0.2      #fixed feed rate
        self._steps = 0
    ...
    def step(self, action):
        ...
        f1=0.2 #fixed feed rate

        delta_h = self.deposition.apply(self.height, x0, y0, x1, y1, f1, self.cfg.dt, use_target_mask=True)
        ...
        viol_outside = np.any(h[outside_box] > 0.02)
...
class ColdSprayExtractor(BaseFeaturesExtractor):
    ...
    def __init__(self, observation_space, features_dim=64, overfill_ratio=1.1):
...
cfg = ColdSprayConfig(grid_n=128, rho_m=6500.0, eta=0.4, target_height=0.2,
                      overfill_ratio=1.01, dx_max=0.1, dy_max=0.1, dt=2.0,
                      max_steps=200000, sigma=0.01, beta=2.0, seed=None)
...
def train_sb3(env_cfg, model, N_epochs=10, log_path="training_log_ppo.txt"):
    ...
    for epoch in range(N_epochs):
        ...
        if rmse < best_rmse:
            ...
            save_exploration_vs_deterministic(
                ...
                outdir="/netscratch/nham/logs/pics-ppo"
            )
...
if __name__ == "__main__":
    ...
    train_sb3(cfg, model, N_epochs=300, log_path="training_log_ppo.txt")