...
class NozzleTrajectoryCallback(BaseCallback):
    """
    Callback để vẽ full rollout (đường đi nozzle) trong lúc training.
    Mỗi rollout = n_steps step của PPO.
    """
    ...
    def __init__(self, save_dir="train_rollouts", verbose=1):
        ...
        self.path_xy = []  # lưu path trong rollout

    # ------------------------------------------------
    def _on_rollout_start(self):
        """Reset buffer khi bắt đầu một rollout PPO."""
        self.path_xy = []
    ...
    # ------------------------------------------------
    def _on_step(self):
        """Log (x,y) mỗi step trong rollout."""
        x = self.training_env.get_attr("x")[0]
        y = self.training_env.get_attr("y")[0]
        self.path_xy.append((x, y))
        return True
    ...
    def _on_rollout_end(self):
        """
        Khi PPO thu thập xong n_steps → rollout kết thúc → vẽ và lưu ảnh path.
        """
        ...
        if self.verbose:
            print(f"✓ Saved training rollout: {fname}")
