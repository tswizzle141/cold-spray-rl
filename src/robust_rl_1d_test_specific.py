...
class ColdSpray1DConfig:
    ...
    use_manual_test_case: bool = True
    manual_target_baseline: float = 0.01
    manual_bump_height: float = 0.2
    manual_bump_left: float = 0.25
    manual_bump_right: float = 0.75
    disable_gp_noise_in_manual_case: bool = True
...
def _sample_target_base_and_vbase(
    ...
    if cfg.use_manual_test_case:
        baseline = float(cfg.manual_target_baseline)
        bump_height = float(cfg.manual_bump_height)
        left = float(cfg.manual_bump_left)
        right = float(cfg.manual_bump_right)

        target_base = np.full_like(x_grid, baseline, dtype=np.float64)

        rect_mask = (x_grid >= left) & (x_grid <= right)
        target_base[rect_mask] = baseline + bump_height
    ...
    # ===== CUSTOM TEST CASE MODIFICATION END =====
...
class ColdSpray1DEnv(gym.Env):
    ...
    def reset(self, *, seed: Optional[int] = None, options: Optional[dict] = None):
        ...
        if self.cfg.use_manual_test_case and self.cfg.disable_gp_noise_in_manual_case:
            self.gp_noise = np.zeros_like(self.x_grid, dtype=np.float32)
        else:
...
def plot_manual_target_debug(
    cfg: ColdSpray1DConfig,
    save_name: str = "manual_target_debug.png",):
    ...
    axes[0].plot(env.x_grid, env.target_base, linewidth=2, label="target_base")
    axes[0].plot(env.x_grid, env.target, "--", linewidth=1.5, label="target")
    axes[0].set_ylabel("height")
    axes[0].set_title("Manual rectangle target")
    axes[0].legend()

    axes[1].plot(env.x_grid, env.v_base_vec, linewidth=2, label="v_base_i")
    axes[1].set_xlabel("x")
    axes[1].set_ylabel("v_base")
    axes[1].set_title("Velocity profile derived from target_base")
    axes[1].legend()

    plt.tight_layout()
    out_path = _save_fig(fig, save_name)
    print(f"[plot_manual_target_debug] Saved figure to: {out_path}")
    return out_path
...
if __name__ == "__main__":
    cfg = ColdSpray1DConfig(
        ...
        vecnormalize_path=str("/netscratch/nham/checkpoints/best_model_9_vecnormalize.pkl"),
        use_random_shape_bumps=True,
        allowed_bump_shapes=("triangle", "trapezoid", "rectangle", "semicircle", "flat"),

        # ===== CUSTOM TEST CASE MODIFICATION START =====
        use_manual_test_case=True,
        manual_target_baseline=0.01,
        manual_bump_height=0.2,
        manual_bump_left=0.25,
        manual_bump_right=0.75,
        disable_gp_noise_in_manual_case=True,
        # ===== CUSTOM TEST CASE MODIFICATION END =====
    )

    plot_manual_target_debug(cfg, save_name="manual_target_debug.png")
