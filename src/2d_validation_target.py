...
Original file is located at
    https://colab.research.google.com/drive/1pzC7HUDgGvrOep8gwPEs0Q40SwNrFWIG
"""
...
# ---------------------------------------------------------------------------
MODEL_PATHS = {
    0.00: Path("/content/drive/MyDrive/thesis/checkpoint/best_model_11.zip"),
    0.05: Path("/content/drive/MyDrive/thesis/checkpoint/best_model_12.zip"),
    0.10: Path("/content/drive/MyDrive/thesis/checkpoint/best_model_12.zip"),
}
VECNORMALIZE_PATHS = {
    0.00: Path("/content/drive/MyDrive/thesis/checkpoint/best_model_11_vecnormalize.pkl"),
    0.05: Path("/content/drive/MyDrive/thesis/checkpoint/best_model_12_vecnormalize.pkl"),
    0.10: Path("/content/drive/MyDrive/thesis/checkpoint/best_model_12_vecnormalize.pkl"),
}
MODEL_PATH = MODEL_PATHS[0.05]
VECNORMALIZE_PATH = VECNORMALIZE_PATHS[0.05]
OUTPUT_DIR = Path("/content/validation_target_gp_2d")
...
def make_fixed_target_bundle(
    amplitude: float,
    cfg_template: ColdSpray2DConfig,
    seed: int,
) -> Tuple[ColdSpray2DConfig, GPTarget2DBundle]:
    """
    Build one target-only GP case.

    h0 is exactly flat because build_gp_target_2d_bundle() fixes its relative
    amplitude to zero in this SB3-12 formulation. Setting min=max forces the
    requested target amplitude instead of drawing it randomly.
    """
    ...
    if not np.allclose(bundle.h0_modification, 0.0):
        raise RuntimeError("Target-only validation requires h0 = 0.")
    return cfg, bundle
...
def _show_target_difference(ax, difference, title, abs_limit):
    """
    Show target_after_GP - nominal_target with a zero-centred colour scale.
...
def plot_targets_and_paths(results: Dict[float, Dict[str, Any]]) -> Path:
    ...
    A single nominal target | GP-induced target difference by amplitude |
    ...
    nominal_maps = [item["target_base"] for item in results.values()]
    vmin = min(float(np.min(x)) for x in nominal_maps)
    vmax = max(float(np.max(x)) for x in nominal_maps)

    differences = {
        amp: results[amp]["target"] - results[amp]["target_base"]
    ...
    _show_height(
        nominal_ax,
        results[AMPLITUDES[0]]["target_base"],
        "Nominal target",
        vmin,
        vmax,
    )
    ...
    difference_axes[-1].set_xlabel(
        "Target difference", fontweight="bold", labelpad=12
    )
    ...
    fig.suptitle(
        "Nominal target, GP-induced difference, and deterministic executed path",
        fontsize=24, fontweight="bold",
    )
    out = OUTPUT_DIR / "figure_2_targets_path.png"
...
def plot_combined_validation(results: Dict[float, Dict[str, Any]]) -> Path:
    ...
    fig.suptitle(
        "Controls, nominal target, GP-induced target difference, "
        "and executed path",
        fontsize=32,
        fontweight="bold",
        y=0.982,
    )
    out = OUTPUT_DIR / "figure_combined_target_gp_validation.png"
...
def save_metrics(results: Dict[float, Dict[str, Any]]) -> Path:
    out = OUTPUT_DIR / "target_gp_validation_metrics.csv"
    rows = [
        "gp_amplitude,predefined_rel_mse,deterministic_rel_mse,"
...
def main() -> None:
    ...
    for amp in AMPLITUDES:
        ...
        print(f"\nRunning target-GP amplitude {100*amp:.0f}% ...")
        print(f"  model: {model_path.name}")
        print(f"  VecNormalize: {vecnormalize_path.name}")
        cfg, bundle = make_fixed_target_bundle(amp, template, VALIDATION_SEED)
