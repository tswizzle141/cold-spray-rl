...
LOG_FILE = open("/netscratch/nham/logs/training_debug_log_ppo_2.txt", "w", encoding="utf-8")
def log(*args):
...
BEST_MODEL_PATH = os.path.join(CKPT_DIR, "best_model_2")
...
class ColdSprayEnv(gym.Env):
    ...
    def reset(self, *, seed=None, options=None):
        ...
        # Build continuous snake polyline (baseline path)
        self._snake_pts, self._snake_cumlen = self._build_snake_polyline(
            box_min=BOX_MIN,
            box_max=BOX_MAX,
            spacing_px=6
        )
        self._snake_total = float(self._snake_cumlen[-1])

        # progress along snake (arc-length) and lateral offset
        self.s = 0.0
        self.off = 0.0

        # initialize nozzle position exactly on baseline start
    ...
    def step(self, action):
        ...
        self.s = float(np.clip(self.s + ds, 0.0, self._snake_total))
        self.off = float(np.clip(self.off + doff, -0.10, 0.10))  # clip off to something reasonable

        # baseline pose at new s
        bx, by, tx, ty = self._snake_pose_at_s(self.s)
...
def plot_det_critic_vs_target_return(epoch: int, det_rewards, det_values, det_dones,
                                     gamma: float, gae_lambda: float, outdir="pics-ppo-2"):
    """
    Plot:
...
cfg = ColdSprayConfig(grid_n=128, rho_m=6500.0, eta=0.4, target_height=0.2,
                      overfill_ratio=1.25, v_max=0.05, off_max=0.02, dt=2.0, 
...
if os.path.exists("/netscratch/nham/checkpoints/best_model_2.zip"):
...
def train_sb3(env_cfg, model, N_epochs=10, log_path="training_log_ppo_2.txt"):
    ...
    for epoch in range(N_epochs):
        ...
        if mean_h_inside > best_mean_h:
            ...
            plot_det_critic_vs_target_return(
                ...
                outdir="/netscratch/nham/logs/pics-ppo-2")
...
if __name__ == "__main__":
    ...
    train_sb3(cfg, model, N_epochs=500, log_path="training_log_ppo_2.txt")