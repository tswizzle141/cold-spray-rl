...
SAVE_DIR = Path("/netscratch/nham/logs/pics-ppo-13")
...
class ColdSpray2DConfig:
    ...
    # Must match sb3_cold_spray_generalized_gaussian_ppo_13(2).py.
    ...
    resume: bool = True
    resume_model_path: str = str(CHECKPOINT_DIR / "best_model_13.zip")
    resume_vecnormalize_path: str = str(CHECKPOINT_DIR / "best_model_13_vecnormalize.pkl")
    save_freq: int = 50000
...
def build_gp_target_2d_bundle(
    ...
    New pipeline requested by the user:
        ...
        6) keep target modulation at zero, as in model 13
    ...
    # File 13 experiment: apply the continuous amplitude to h0.
    sampled_h0_mod_relative_amplitude = float(
        rng.uniform(
    ...
    # Match file 13 exactly: still sample g_mod (preserving RNG progression),
    # but multiply it by a target amplitude fixed to zero.
    ...
    sampled_target_mod_relative_amplitude = 0
    target_modulation = (
...
# =============================================================================
# Standalone 2D h0-GP amplitude sensitivity analysis (model 13)
# =============================================================================

# ----------------------------- USER SETTINGS -----------------------------
# Server paths.
CHECKPOINT_DIR = Path("/netscratch/nham/checkpoints")
MODEL_PREFIX = "best_model_13"
OUT_DIR = Path("/netscratch/nham/logs/pics-ppo-13")
...
def make_analysis_config(amplitude_relative: float) -> ColdSpray2DConfig:
    """Return model-13 configuration with one fixed h0-GP amplitude."""
...
def load_analysis_model() -> Tuple[PPO, VecNormalize]:
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "sb3_cold_spray_generalized_gaussian_ppo_13(2).py. Verify that "
...
def evaluate_one_rollout(
    ...
    # Fix all randomness belonging to this rollout. The environment samples
    # independent g_base and g_h0 fields; g_mod is identically zero in model 13.
    ...
    # This experiment varies h0 GP only; target modulation must remain zero.
    if not np.isclose(
        env.sampled_h0_mod_relative_amplitude,
        amplitude_relative,
        rtol=0.0,
        atol=1e-12,
    ):
        raise AssertionError("The sampled h0 amplitude is not the requested fixed level.")
    if abs(float(env.sampled_target_mod_relative_amplitude)) > 1e-12:
        raise AssertionError("Target GP amplitude must be exactly 0 in h0-GP analysis.")
    if float(np.max(np.abs(env.target_modulation))) > 1e-10:
        raise AssertionError("Target modulation must be exactly zero in h0-GP analysis.")
    ...
    return {
        ...
        "seed": int(env_seed),
        "h0_GP_amplitude_percent": 100.0 * float(amplitude_relative),
        "target_GP_amplitude_percent": 0.0,
...
def run_sweep(model: PPO, venv: VecNormalize) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for amp_percent in AMPLITUDES_PERCENT:
        ...
        print(f"Evaluating h0 GP amplitude {amp_percent:.2f}% "
...
def save_results(raw_df: pd.DataFrame, summary_df: pd.DataFrame) -> Tuple[Path, Path]:
    raw_path = OUT_DIR / f"2d_h0_gp_amp_raw_{MODEL_PREFIX}.csv"
    summary_path = OUT_DIR / f"2d_h0_gp_amp_summary_{MODEL_PREFIX}.csv"
...
def plot_diagram(raw_df: pd.DataFrame, summary_df: pd.DataFrame) -> Tuple[Path, Path]:
    ...
    # amplitude levels, so denser sweeps retain enough horizontal spacing.
    ...
    # Alternate mean labels above and below their markers. Four offset levels
    # avoid creating one dense row when neighbouring means are similar. Labels
    # close to y=0 are forced upward so they never collide with the x-axis.
    ...
    lower_safety_level = 0.065 * visible_top
    upper_offsets = (16, 34)
    lower_offsets = (-14, -24)

    for idx, (xi, yi) in enumerate(zip(x, mean)):
    ...
    ax.set_xlabel("Relative h0 GP amplitude (%)", fontsize=18, labelpad=12)
    ax.set_ylabel("Relative MSE", fontsize=18, labelpad=12)
    ax.set_title(
        "Individual runs + mean ± std\n"
        f"Relative h0 GP amplitude vs relative MSE "
    ...
    png_path = OUT_DIR / f"2d_h0_gp_amp_relative_mse_{MODEL_PREFIX}.png"
    pdf_path = OUT_DIR / f"2d_h0_gp_amp_relative_mse_{MODEL_PREFIX}.pdf"
...
if __name__ == "__main__":
    print("2D h0-GP amplitude analysis; target GP amplitude is fixed at 0%.")
