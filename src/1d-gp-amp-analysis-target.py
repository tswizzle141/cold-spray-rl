...
MODEL_PREFIX = "best_model_4"   # change to another target-GP checkpoint if needed
OUT_DIR = Path("/content/target_gp_sensitivity_outputs")
OUT_DIR.mkdir(parents=True, exist_ok=True)

# Sweep target-GP amplitude from 0% to 10% of mean(target_base).
# h0_GP_amplitude is fixed at exactly 0% for every rollout.
AMPLITUDES_PERCENT = np.arange(0.0, 10.0 + 0.5, 0.5, dtype=np.float64)
H0_RELATIVE_AMPLITUDE = 0.0

# Keep this value as in your original script. Set to 16 for a quick test.
...
print("[Setup] Sensitivity analysis on TARGET GP amplitude; h0_GP_amplitude = 0.0% for all rollouts.")
...
def build_fixed_target_gp_profiles(
    cfg: ColdSpray1DConfig,
    rng: np.random.Generator,
    target_relative_amplitude: float,
) -> GPProfiles:
    """Build one fixed-amplitude test profile.

    The nominal target is generated from a flat initial surface. The GP amplitude
    is then applied to the target, while h0 remains exactly flat:

        h0(x) = 0,
        target(x) = max(target_base(x)
                        + a_target * mean(target_base) * g_target(x), 0).
    """
    x_grid = np.linspace(0.0, cfg.x_max, cfg.n_bins, dtype=np.float32)
    dx = float(x_grid[1] - x_grid[0])

    # GP for nominal base velocity, unchanged from the original sensitivity script.
    ...
    # h0 is forced flat in the target-GP experiment.
    h0_modification = np.zeros_like(x_grid, dtype=np.float32)

    # GP perturbation applied to target, not to h0.
    g_target = sample_zero_mean_gp(rng, x_grid, cfg.gp_std, cfg.gp_lengthscale, cfg.gp_jitter)
    target_modulation = (
        float(target_relative_amplitude)
        * float(np.mean(target_base.astype(np.float64)))
        * g_target.astype(np.float64)
    ).astype(np.float32)
    ...
    return GPProfiles(
        ...
        g_base=g_base.copy(),
        g_target=g_target.copy(),
        sampled_target_mod_relative_amplitude=float(target_relative_amplitude),
...
class ColdSpray1DTestEnv(gym.Env):
    ...
    def __init__(
        self,
        cfg: ColdSpray1DConfig,
        target_relative_amplitude: float = 0.03,
        env_seed: int = 0,
    ):
        super().__init__()
        self.cfg = cfg
        self.target_relative_amplitude = float(target_relative_amplitude)
    ...
    def _set_profiles(self) -> None:
        profiles = build_fixed_target_gp_profiles(
            self.cfg,
            self._rng,
            self.target_relative_amplitude,
        )
        self.v_base_vec = profiles.v_base_vec.copy()
        self.target_base = profiles.target_base.copy()
        self.target = profiles.target.copy()
        self.target_modulation = profiles.target_modulation.copy()
        self.h0_modification = profiles.h0_modification.copy()
        self.baseline_deposition = profiles.baseline_deposition.copy()
        self.g_base = profiles.g_base.copy()
        self.g_target = profiles.g_target.copy()
        self.sampled_target_mod_relative_amplitude = float(profiles.sampled_target_mod_relative_amplitude)
        self.sampled_h0_mod_relative_amplitude = float(profiles.sampled_h0_mod_relative_amplitude)
    ...
    def reset(self, *, seed: Optional[int] = None, options: Optional[dict] = None):
        ...
        # Critical change: the rollout starts from flat h0, not from a GP-perturbed h0.
        self.height = np.zeros_like(self.h0_modification, dtype=np.float32)
        self.nozzle_x = 0.0
    ...
    def step(self, action: np.ndarray):
        ...
        info = {
            ...
            "dv": float(dv),
            "target_relative_amplitude": float(self.target_relative_amplitude),
            "h0_relative_amplitude": float(H0_RELATIVE_AMPLITUDE),
            "h0_std": float(np.std(self.h0_modification)),
...
def make_base_vec_env_for_loading() -> DummyVecEnv:
    def _factory():
        return Monitor(ColdSpray1DTestEnv(cfg=cfg, target_relative_amplitude=0.0, env_seed=123))
...
def evaluate_one_rollout(
    ...
    env = ColdSpray1DTestEnv(
        cfg=cfg,
        target_relative_amplitude=target_relative_amplitude,
        env_seed=env_seed,
    )
    obs, _ = env.reset()

    # Safety check: h0 must be exactly flat for this experiment.
    h0_std = float(np.std(env.h0_modification))
    if h0_std > 1e-12:
        raise AssertionError(f"h0 must be flat, but h0_std={h0_std:.6e}")
    ...
    return {
        ...
        "seed": int(env_seed),
        "target_relative_amplitude": float(target_relative_amplitude),
        "h0_relative_amplitude": float(H0_RELATIVE_AMPLITUDE),
        "h0_std": h0_std,
...
for amp_percent in AMPLITUDES_PERCENT:
    ...
    print(f"Evaluating TARGET amplitude {amp_percent:.1f}% ({N_TESTS_PER_AMPLITUDE} runs), h0 amplitude = 0.0%...")
    for run_idx in range(N_TESTS_PER_AMPLITUDE):
        ...
        out["target_GP_amplitude_percent"] = float(amp_percent)
        out["h0_GP_amplitude_percent"] = 0.0
...
if float(raw_df["h0_std"].max()) > 1e-12:
    raise AssertionError("h0 is not flat in at least one rollout.")

raw_csv_path = OUT_DIR / f"target_gp_sensitivity_raw_{MODEL_PREFIX}.csv"
summary_csv_path = OUT_DIR / f"target_gp_sensitivity_summary_{MODEL_PREFIX}.csv"
...
def plot_target_gp_sensitivity_figure(
    raw_df: pd.DataFrame,
    ...
    ax.set_xlabel("Target GP amplitude (% of mean target_base)")
    ax.set_ylabel("relative MSE")
    ax.set_title(
        f"Individual runs + mean ± std\n"
        f"Target GP amplitude (% of mean target_base) vs relative MSE "
    ...
    png_path = out_dir / f"target_gp_sensitivity_relative_mse_{model_prefix}.png"
    pdf_path = out_dir / f"target_gp_sensitivity_relative_mse_{model_prefix}.pdf"
...
def plot_slide_style_figure(
    ...
    header_ax.text(
        0.01, 0.55,
        "TARGET-GP AMPLITUDE SENSITIVITY",
        color="white",
        fontsize=28,
        va="center",
        ha="left",
    ...
    caption_ax.text(0.41, 0.5, " Target-GP amplitude sensitivity", color="black", fontsize=26, ha="left", va="center")

    png_path = out_dir / f"target_gp_sensitivity_relative_mse_slide_style_{model_prefix}.png"
    pdf_path = out_dir / f"target_gp_sensitivity_relative_mse_slide_style_{model_prefix}.pdf"
    fig.savefig(png_path, dpi=FIG_DPI, bbox_inches="tight")
    fig.savefig(pdf_path, bbox_inches="tight")
    plt.show()
    return png_path, pdf_path

png_path, pdf_path = plot_target_gp_sensitivity_figure(raw_df, summary_df, OUT_DIR, MODEL_PREFIX)
